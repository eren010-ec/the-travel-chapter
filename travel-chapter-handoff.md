...
## Workstream R — Netlify credit reset, both sites redeployed (added 2026-08-26)

User ran `netlify login` (fresh browser OAuth) then said the team's Netlify deploy credit had reset and asked to push/deploy everything outstanding. Two problems surfaced before any deploy happened:

1. **Nothing was actually unpushed at the git level** — `main` was already even with `origin/main` at `b0744c2` (everything through Workstream Q had been pushed already; it just never *built* on Netlify, per the credit-exhaustion note below).
2. **The fresh `netlify login` authenticated the wrong Netlify account.** `netlify status`/`netlify sites:list` looked plausible (a team literally named "The Chapter") but only showed one unrelated site (`orvixhr`) — neither `thetravelchapter` nor `quietmeridian-4471` existed under that login. Confirmed via `netlify api listSitesForAccount` (not just `sites:list`, which can be misleadingly scoped). This looked at first like the sites had been deleted, but the user provided a personal access token for the correct account, and `netlify api listSites --auth <token>` immediately showed both real sites (`thetravelchapter`, siteId `56615704-86bc-4072-9a46-a1733c80eb8e`, domain `thetravelchapter.my` still attached; `quietmeridian-4471`, siteId `c5f73ef7-2043-4f04-8611-702f5a4e773b`) plus `yakinikudong`. So: **the sites were never gone, just a wrong-account CLI login** — see [[project_netlify_account_mismatch]] for the reusable pattern. The token itself was not persisted anywhere (not in this doc, not committed).

**Confirmed the credit exhaustion via `netlify api listSiteDeploys`:** every customer-site deploy attempt from `73086c1` (2026-08-12) through `b0744c2` (2026-08-26, today's login) had `state: error`, `skipped: true`, `"Skipped due to account credit usage exceeded"`. The last deploy that actually went live was `98bfb904` on 2026-08-06 — so the site had been stuck on a 20-day-old build the entire time despite every intervening `git push` succeeding.

**Deployed, using the auth-token workaround (`--auth <token>` / `NETLIFY_AUTH_TOKEN` env var passed to `netlify api` / `netlify deploy`, not the broken CLI session):**
1. Customer site: no new commit to push, so triggered a fresh rebuild of the current `main` HEAD directly via `netlify api createSiteBuild --data '{"site_id":"56615704-86bc-4072-9a46-a1733c80eb8e"}'`. Polled `getSiteDeploy` until `state: ready` with no error/skip — confirmed live, built from `b0744c2`.
2. Admin site: `netlify deploy --prod --dir=admin --site c5f73ef7-2043-4f04-8611-702f5a4e773b` — this finally shipped the `.my`-domain cross-link corrections from Workstream Q (commit `27c8268`) that had been sitting undeployed since 2026-08-13. Deploy reported `Deploy is live!`.

**Both sites are now current with everything in this doc through Workstream Q.** Per [[feedback_netlify_deploy_gate]], both deploy actions were confirmed with the user first (the admin one specifically needed an explicit approve-and-retry after Claude Code's own auto-mode classifier blocked the first attempt at a bare production deploy command).
...
1. **WhatsApp number** — Content editor → **Contact** → WhatsApp Number. Until set, the trip-card / trip-page WhatsApp button falls back to the phone number. **(Contact CMS now has real values as of the follow-ups — `a1.thetravelchapter@gmail.com` / `+6013 837 2222` — but `whatsapp` is still blank, so the button uses the phone.)**
...
### Follow-ups shipped the same day (2026-09-03, commits `64e3b7d` → `97ce00a`)
Six small commits after `4ada99d`, all pushed; customer bits auto-deployed, one admin deploy shipped `64e3b7d`+`4a28d2e`+`f731d3b`+`2302023` together (`netlify deploy --prod --dir=admin`, `NETLIFY_AUTH_TOKEN=<PAT>` — the classifier let it through this time). All verified live.

- **`64e3b7d` — Admin trips: allow "TBA".** `parseInt('')`/`parseFloat('')` on a cleared Duration/Price/Max Pax field returned `NaN` → `null` → `"duration_days violates not-null constraint"` on save. Now `parseInt(...) || 0` / `parseFloat(...) || 0` for all three. Fields' `min` relaxed to 0, `placeholder="0 = TBA"`, + a hint line under them. The public pages already treat `0`/empty as "not set" → shows "Price TBA", hides the duration/group-size rows, disabled "TBA" button when `departure_dates` is empty. **To make a TBA trip: Title (+ Slug), set Duration/Price/Max Pax to 0, leave Departure Dates empty.**
- **`7bd9105` — Footer contact email/phone now read from CMS.** The footer's Contact column had hardcoded `hello@thetravelchapter.com` / `+60 3 1234 5678` on every page; editing Email/Phone in CMS → Contact only updated `contact.html`'s in-page block. Added `#footer-email` / `#footer-phone` ids on all 6 customer pages and wired them from `cms_content.contact`: index (via `applyFooterContact(cms.contact)` in `loadCMS`), trips/trip (in `loadContact()`), contact (in its existing `loadContact()`), about (`loadFooter()` extended to `.in(['footer','contact'])`), free-gifts (new `loadFooterContact()`). `sw-customer.js` cache **v12 → v13**.
- **`4a28d2e` — Admin Trips filters.** Table toolbar gained `.filter-select`s: **Status** (Active / Hidden / **TBA – no price or dates**), **Destination** (distinct `region`), **Category** (distinct `category`, both auto-populated from `allTrips`), + a **Clear** button — all combine with the search box. `renderTripsTable()` now `fillTripFilterOptions(); filterTrips();` so filters stay applied after save/delete (was `renderTripsRows(allTrips)`). Guarded the search against null `t.title`.
- **`f731d3b` — Notification chime cadence 60s → 10s.** Split the poll: data still re-fetches every 60s; a separate `setInterval(() => { if (notifTotal > 0) maybeChime(); }, 10000)` re-fires the sound (no network). The 2s `maybeChime()` dedupe still prevents overlap with the new-item trigger.
- **`2302023` — Bell sound = user-provided MP3.** User dropped `service-bell-sound-soundeffects-asmr.mp3` in the repo root; moved to **`admin/notification-bell.mp3`** (4.0s, stereo 48kHz, ~63KB — the admin site only deploys `--dir=admin`). `playChime()` now `new Audio('notification-bell.mp3')` (created once) `.currentTime=0; .play().catch()`; the WebAudio oscillator code + `notifAudioCtx` are gone. `armAudio()` (first `pointerdown`/`keydown`) now primes with a muted play/pause so the timed `.play()` is allowed. `sw-admin.js` cache **v2 → v3**, mp3 in APP_SHELL. ⚠️ 4s every 10s is relentless — mute toggle in the bell dropdown is the escape hatch.
- **`97ce00a` — Contact form: Phone field beside Email.** `#cf-email` + new `#cf-phone` (`type="tel"`, optional, `placeholder +60 12 345 6789`) are now a 2-col `.form-row` like First/Last Name. Submit handler sends its value as `phone` (was hardcoded `null`); `contact_messages` + the admin Messages tab already display phone. New i18n `contact.form.phone` / `contact.form.phone_ph` (EN/ZH/MS).
...
- [x] ~~Netlify production deploys are fully paused (customer site, discovered 2026-08-12) — the team's billing-cycle deploy credits are exhausted.~~ **Resolved 2026-08-26 (Workstream R)** — credit reset, customer site rebuilt and live at `b0744c2` via `netlify api createSiteBuild`. (Historical detail kept below for context.) Confirmed via Netlify dashboard (`app.netlify.com/projects/thetravelchapter/deploys`): every deploy attempt since `98bfb90` (2026-08-06!) showed status "Skipped — Skipped due to account credit usage exceeded", including all 3 pushes from that day (`73086c1`, `ff6ecfa`, `d07de4f`). The live customer site was stuck on the 2026-08-06 build for 20 days despite every `git push` succeeding at the git level — this blocked **both** deploy paths (git-triggered auto-deploy AND manual `netlify deploy` CLI), not just the admin one.
- [x] New commit `06683c3` (Twilio Verify + `+60` prefill, Workstream I) and the Workstream K changes (registration email field, admin phone/email visibility) — pushed and now live as of the 2026-08-26 rebuild (Workstream R).
...
- [x] Redeploy `admin/` to the admin Netlify site (`netlify deploy --prod --dir=admin --site c5f73ef7-2043-4f04-8611-702f5a4e773b`) so its corrected cross-links (Workstream Q) go live — **done 2026-08-26 (Workstream R)**, `Deploy is live!`.
